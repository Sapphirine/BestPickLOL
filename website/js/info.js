...
function dashboard(id, fData) {
    ...
    function segColor(c) {
        cmap = {Win: "#4753CC", Lose: "#828499"};
        return cmap[c]; /* TO FINISH */
    }

    // compute total for each state.
    fData.forEach(function (d) {
        d.total = d.count.Win + d.count.Lose; /* TO FINISH */
    });

    // function to handle histogram.
    function histoGram(fD) {
        var hG = {}, hGDim = {t: 60, r: 0, b: 30, l: 0};
        hGDim.w = 2.5*(500 - hGDim.l - hGDim.r),
        ...
        bars.append("rect")
            .attr("x", function (d,i) {
                return 1*i * (x.rangeBand()+5)+20}) /* TO FINISH */
            .attr("y", function (d,i) {
                return y(d[1]);}) /* TO FINISH */
        ...
        bars.append("text").text(function (d) {
            ...
            .attr("x", function (d,i) {
                return 1*(i+0.5) * (x.rangeBand()+5)+13;}) /* TO FINISH */
            .attr("y", function (d,i) {
                return y(d[1]) - 5;}) /* TO FINISH */
            .attr("text-anchor", "middle");

        function mouseover(d) {  // utility function to be called on mouseover.
            // filter for selected state.
            var st = fData.filter(function (s) {
                    return s.ChampionID == d[0];
                })[0],
    ...
    function pieChart(pD) {
        ...
        function mouseover(d) {
            ...
            hG.update(fData.map(function (v) {
                return [v.ChampionID, v.count[d.data.type]];
    ...
    var tF = ['Win','Lose'].map(function (d) {
        return {
            type: d, count: d3.sum(fData.map(function (t) {
                return t[d] /* TO FINISH */;
            }))
        };
    });
